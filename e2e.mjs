...
import { DOCUMENT_ORDER } from './src/chapters.js'
...
const EXEC = { agent: { id: process.env.SESSION_ID ?? 'e2e-session' } }
...
const got = await info.execute({ planId: PLAN_ID }, EXEC)
if (got.data !== undefined) {
  throw new Error('取数工具不应把完整 data 放进结果')
}
if (!existsSync(got.filePath)) {
  throw new Error(`方案 JSON 未落盘: ${got.filePath}`)
}
const data = JSON.parse(readFileSync(got.filePath, 'utf8'))
...
const page = await reader.execute({ path: got.path, offset: 1, limit: 20 }, {})
...
const tpl = await reader.execute({ path: '01.html' }, {})
...
for (const no of DOCUMENT_ORDER) {
  ...
  last = await write.execute({ chapterNo: no, content }, EXEC)
